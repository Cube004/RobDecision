...
public:
    ...
    DecisionCore(DecisionConfig decision_config, ros::NodeHandle *nh):decision_config_(decision_config), nh_(nh){
        ...
        (nh, decision_config_.topic_cancel_goal_, decision_config_.topic_send_goal_, 
        decision_config_.frame_id_, decision_config_.child_frame_id_);
        this->init();
    }
    ...
    void decision(){
        ...
        while(this->run_decision_){
            ...
            ROS_INFO("target_node_id = %d, duration = %f", this->decision_result_.target_id_, this->decision_result_.duration_.toSec());
    ...
    DecisionResult getTargetNode(){
        ...
        auto edges = &this->graph_.nodeList[this->graph_.rootNode_id].edges;
        auto node_id = this->graph_.rootNode_id;
        bool result = 1;

        while(this->graph_.nodeList[node_id].get_finish() == true){
            ...
            for(auto edge : *edges){
                ...
                for(auto condition : edge.rules.conditions){
                    ...
                    if (condition.temporal_scope == rules::temporalScope::TASK_CYCLE){// 任务周期情况, 用于监控任务完成后一段时间的数值变化
                        if (graph_.nodeList[node_id].get_finish() == true)
                        {
                            duration = ros::Time::now() - graph_.nodeList[node_id].end_time;
                        }else{
                            break;
                        }
                    }
                ...
                if(result){// 如果所有条件都满足，则跳转到下一节点
                    ...
                    edges = &this->graph_.nodeList[node_id].edges;
                    break;
