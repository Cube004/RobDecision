...
class Graph {
    public:
        ...
        std::unordered_map<int, std::vector<int>> nodeGroup;
        std::string decisionJsonStr;
        int rootNode_id = -1;
        bool init(const std::string& decisionJsonStr);
        void AddNodeGroup(Json::Value nodeGroup);
    ...
    bool Graph::CheckFinish(int id){
        ...
        for (auto& groupIndex : this->nodeList[id]->group_id) {
            ...
            std::cout << "nodes: ";
            for (auto& node_id : this->nodeGroup[groupIndex]) {
                std::cout << node_id << " ";
    ...
    void Graph::loadJson(std::string decisionJsonStr){
        ...
        Json::Value nodeGroup = root["NodeGroup"];
        // 添加节点
        for (auto& node : nodes) {
            this->AddNode(node);
        }
    ...
    void Graph::AddNodeGroup(Json::Value nodeGroup){
        std::vector<int> nodes;
        for (auto& group : nodeGroup) {
            for (auto& node : group["nodes"]) {
                nodes.push_back(node.asInt());
                this->nodeList[node.asInt()]->group_id.push_back(group["id"].asInt());
            }
            this->nodeGroup[group["id"].asInt()] = nodes;
        }
    }
    ...
    void Graph::AddEdge(Json::Value edge){
        ...
        newEdge->weight = std::stoi(this->ParseJsonToString(edge["condition"]["weight"], "0"));
    ...
    void Graph::Debuginfo(){
        for (const auto& node : this->nodeList) {
            ...
            for (const auto& group_id : node.second->group_id) {
                std::cout << "group_id: " << group_id;
                std::cout << "nodes: ";
                for (const auto& node_id : this->nodeGroup[group_id]) {
                    std::cout << node_id << " ";
                }
            }
            std::cout << std::endl;
